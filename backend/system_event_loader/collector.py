...
class SystemCollector:
    ...
    def __init__(self, db_path="telemetry_edge.db"):
    ...
    # 2. OS EVENT LOGS
    # -------------------------------------------------------------------------
    def get_system_logs(self, max_records=10) -> list:
    ...
    def _get_linux_logs(self, max_records) -> list:
        events = []
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    ...
                    formatted_ts = None
                    if raw_ts:
                        formatted_ts = datetime.fromtimestamp(int(raw_ts) / 1000000, tz=timezone.utc).isoformat()
    ...
    def _get_windows_logs(self, max_records) -> list:
        if not win32evtlog:
    ...
    def _get_windows_peripherals(self) -> dict:
        ...
        try:
            ...
            cmd_u = "Get-PnpDevice -PresentOnly | Where-Object { $_.Class -match 'USB|Printer|Image' } | Select-Object FriendlyName, InstanceId, Status | ConvertTo-Json"
    ...
    def _get_linux_peripherals(self) -> dict:
        ...
        try:
            ...
            if res_usb.returncode == 0:
                devices["usb_devices"] = [line for line in res_usb.stdout.strip().split("\n") if line]
